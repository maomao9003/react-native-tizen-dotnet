...
class AppStateSubscription extends React.Component {
  ...
  render() {
    if (this.props.showMemoryWarnings) {
      return (
        <View>
          <Text style={{color:'black'}}>{this.state.memoryWarnings}</Text>
        </View>
      );
    }
    if (this.props.showCurrentOnly) {
      return (
        <View>
          <Text style={{color:'black'}}>{this.state.appState}</Text>
        </View>
      );
    }
    return (
      <View>
        <Text style={{color:'black'}}>{JSON.stringify(this.state.previousAppStates)}</Text>
      </View>
    );
  }
}
...
exports.examples = [
  {
    ...
    render() { return <Text style={{color:'black'}}>{AppState.currentState}</Text>; }
  },
  {
