  ...
  render() {
    var button = this.state.status === 'Downloading...' ? (
      ...
      <TouchableHighlight
        ...
        <View style={styles.button}>
         <Text style={{color:'black'}}>Get headers</Text>
        </View>
      </TouchableHighlight>
    );
    ...
    return (
      <View>
        {button}
        <Text style={{color:'black',width: 1300, height: 90}}>headers: {this.state.headers}</Text>
        <Text style={{color:'black'}}>status: {this.state.status}</Text>
      </View>
    );
  }
}
